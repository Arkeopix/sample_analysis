...
** Propagation
   ...
   in order to find a USB device and copy itself on it. Actually, the =DriveType= property indicates
   that the device has "no root directory", which i'm not sure about the exact meaning, however, usb devices seems
   to fall in this category (based on some testing i've done), so i'm not completely out of touch.
